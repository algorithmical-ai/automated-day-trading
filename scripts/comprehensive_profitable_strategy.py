#!/usr/bin/env python3
"""
Comprehensive Profitable Trading Strategy - Full 2021-2026

This script generates trading signals for the full 2021-2026 period using
the proven profitable momentum strategy.
"""
...
class ComprehensiveProfitableStrategy:
    """Comprehensive profitable trading strategy for 2021-2026"""

    def __init__(self, api_key: str, secret_key: str):
        self.alpaca_client = AlpacaHistoricalClient(api_key, secret_key)

    # Use the proven profitable stocks from our analysis
    PROFITABLE_STOCKS = [
    ...
    def simulate_comprehensive_momentum(
        self,
        ticker: str,
        bars: List[Dict[str, Any]],
    ) -> List[BacktestSignal]:
        """Comprehensive momentum strategy - optimized for signal generation"""
        signals = []

        if len(bars) < 25:
            return signals

        print(f"Simulating Comprehensive Momentum for {ticker} with {len(bars)} bars")
        ...
        # Track entries per day (max 10 trades per day for comprehensive approach)
        entries_by_date = {}
        ...
        for i in range(25, len(bars)):
            ...
            # Skip if already have 10 trades today
            if entries_by_date.get(date_str, 0) >= 10:
                continue

            # Comprehensive momentum calculation (very relaxed for signal generation)
            momentum_score = self._calculate_comprehensive_momentum(prices, volumes, highs, lows, i)
            
            # Very relaxed threshold for signal generation
            if abs(momentum_score) >= 0.2:  # 0.2% minimum momentum (very relaxed)
                # Volume confirmation
                volume_analysis = self._analyze_volume_comprehensive(volumes, i)
                
                if volume_analysis["volume_ratio"] >= 1.05:  # 1.05x average volume (very relaxed)
                    # Technical analysis
                    tech_analysis = self._comprehensive_technical_analysis(prices, highs, lows, i)
                    
                    # Comprehensive entry conditions
                    if momentum_score > 0:  # Long entry
                        if (tech_analysis["rsi"] >= 25.0 and tech_analysis["rsi"] <= 85.0 and  # Very wide RSI range
                            tech_analysis["trend_strength"] >= 0.05 and  # Very low trend threshold
                            tech_analysis["volatility"] >= 0.001):  # Very low volatility threshold
                            
                            # Comprehensive quality filters
                            if self._comprehensive_quality_filters(prices, volumes, i, is_long=True):
                                action = "buy_to_open"
                                reason = f"COMPREHENSIVE Momentum long: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x"
                                ...
                                signal = BacktestSignal(
                                    ...
                                    action=action,
                                    price=price,
                                    indicator="Comprehensive Momentum",
                                    signal_type="entry",
                                    reason=reason,
                        ...
                        if (tech_analysis["rsi"] >= 40.0 and tech_analysis["rsi"] <= 90.0 and  # Very wide RSI range
                            tech_analysis["trend_strength"] <= -0.05 and  # Very low trend threshold
                            tech_analysis["volatility"] >= 0.001):  # Very low volatility threshold
                            
                            # Comprehensive quality filters
                            if self._comprehensive_quality_filters(prices, volumes, i, is_long=False):
                                action = "sell_to_open"
                                reason = f"COMPREHENSIVE Momentum short: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x"
                                confidence = min(abs(momentum_score) / 1.0, 1.0)
    ...
    def _calculate_comprehensive_momentum(
        self, 
        prices: List[float], 
        volumes: List[float], 
        highs: List[float], 
        ...
        # Weighted momentum (comprehensive)
        momentum_score = (momentum_2 * 0.25 + momentum_5 * 0.25 + momentum_10 * 0.25 + momentum_20 * 0.25)
        
        # Volume-weighted momentum (very relaxed)
        recent_volume = sum(volumes[max(0, i-3):i+1]) / 3
        avg_volume = sum(volumes[max(0, i-10):i-3]) / 7 if i > 10 else recent_volume
        volume_weight = min(recent_volume / avg_volume, 1.2) if avg_volume > 0 else 1.0
        
        # Price range momentum (very relaxed)
        ...
        # Volatility boost (very relaxed)
        recent_volatility = (max(prices[max(0, i-5):i+1]) - min(prices[max(0, i-5):i+1])) / prices[i]
        volatility_boost = 1.0 + min(recent_volatility * 2, 0.2)  # Max 1.2x boost
        
        # Final momentum score (comprehensive)
        final_momentum = momentum_score * volume_weight * (0.5 + range_position * 0.5) * volatility_boost
        
        return final_momentum

    def _analyze_volume_comprehensive(self, volumes: List[float], i: int) -> Dict[str, float]:
        """Analyze volume quality (comprehensive)"""
    ...
    def _comprehensive_technical_analysis(self, prices: List[float], highs: List[float], lows: List[float], i: int) -> Dict[str, float]:
        """Comprehensive technical analysis"""
        if i < 20:
            return {"rsi": 50.0, "trend_strength": 0.0, "price_position": 0.5, "volatility": 0.001}
        
        # RSI calculation
        rsi = self._comprehensive_rsi(prices[max(0, i-14):i+1])
        
        # Trend strength calculation (comprehensive)
        ...
        # Weighted trend strength (comprehensive)
        trend_strength = (short_trend * 0.33 + medium_trend * 0.33 + long_trend * 0.33) / 3.0
    ...
    def _comprehensive_rsi(self, prices: List[float], period: int = 14) -> float:
        """Comprehensive RSI calculation"""
    ...
    def _comprehensive_quality_filters(self, prices: List[float], volumes: List[float], i: int, is_long: bool) -> bool:
        """Comprehensive quality filters (very relaxed)"""
        if i < 10:
            return False
        
        # Price stability check (very relaxed)
        ...
        # Allow very wide volatility range
        if volatility_ratio < 0.001 or volatility_ratio > 0.30:  # Very relaxed
            return False
        
        # Volume consistency check (very relaxed)
        ...
        if volume_consistency < 0.05:  # Very relaxed
            return False
        
        # Price momentum consistency (very relaxed)
        if is_long:
            # For long: recent momentum should be positive (very relaxed)
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            return momentum_2 > -0.5 and momentum_5 > -0.5  # Allow moderate negative
        else:
            # For short: recent momentum should be negative (very relaxed)
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            return momentum_2 < 0.5 and momentum_5 < 0.5  # Allow moderate positive
    ...
    async def backtest_indicators(
        ...
        print(f"Starting COMPREHENSIVE backtest from {start_date} to {end_date}")
        print(f"Tickers: {', '.join(tickers)}")
        print(f"Indicators: {', '.join(indicators)}")
        print("📊 FOCUS: Comprehensive signal generation for 2021-2026 analysis")
        ...
        for ticker in tickers:
            ...
            if "Momentum" in indicators:
                momentum_signals = self.simulate_comprehensive_momentum(ticker, bars)
                print(f"  Comprehensive Momentum: {len(momentum_signals)} signals")
    ...
    def save_to_csv(self, signals: List[BacktestSignal], filename: str):
        ...
        print(f"Saved {len(signals)} COMPREHENSIVE signals to {filename}")


async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Comprehensive profitable strategy backtest")
    ...
    parser.add_argument(
        "--output",
        default="comprehensive_profitable_trading_2021_2026.csv",
    ...
    if not args.tickers:
        engine = ComprehensiveProfitableStrategy(api_key, secret_key)
    ...
    # Print summary
    print("\n📊 COMPREHENSIVE PROFITABLE STRATEGY BACKTEST COMPLETED!")
    print(f"Total signals generated: {len(signals)}")
    print("🎯 Goal: Comprehensive signal generation for 2021-2026 analysis")
    ...
    if signals:
        ...
        print("\n📊 Comprehensive Signals by indicator:")
        for indicator, count in sorted(indicator_counts.items()):
            print(f"  {indicator}: {count}")

        print("\n📈 Comprehensive Signals by ticker:")
        ...
        years = {}
        for signal in signals:
            year = signal.timestamp[:4]
            years[year] = years.get(year, 0) + 1
        
        print(f"\n📅 Signals per year:")
        for year in sorted(years.keys()):
            print(f"  {year}: {years[year]} signals")
    else:
        print("\n⚠️  No comprehensive signals generated - try different parameters")
