#!/usr/bin/env python3
"""
Moderate Profitable Trading Strategy with Complete Trade Pairs

This script implements a moderate profitable trading strategy that:
1. Generates fewer but still profitable trades
2. Creates complete trade pairs (entry + exit)
3. Outputs each action as separate CSV rows
4. Balances quality with quantity
"""
...
class ModerateProfitableStrategy:
    """Moderate profitable trading strategy"""

    def __init__(self, api_key: str, secret_key: str):
        self.alpaca_client = AlpacaHistoricalClient(api_key, secret_key)

    # Stocks for moderate strategy
    MODERATE_STOCKS = [
        "AAPL", "TSLA", "NVDA", "AMD", "META", "NFLX", "AMZN", "GOOGL", "MSFT", "SPY"
    ]

    def simulate_moderate_momentum_trades(
        self,
        ticker: str,
        bars: List[Dict[str, Any]],
    ) -> List[TradeAction]:
        """Simulate moderate momentum trades with complete entry/exit pairs"""
        actions = []

        if len(bars) < 30:
            return actions

        print(f"Simulating Moderate Momentum for {ticker} with {len(bars)} bars")
        ...
        # Track active trades and moderate filters
        active_trades = {}
        entries_by_date = {}
        trade_counter = 0
        ...
        for i in range(20, len(bars)):
            ...
            # Moderate entry conditions (balanced)
            entry_signal = self._check_moderate_entry(
            ...
            for trade_id, trade_info in active_trades.items():
                exit_signal = self._check_moderate_exit(
    ...
    def _check_moderate_entry(
        ...
        """Check for moderate entry conditions (balanced)"""
        
        # Max 3 trades per day
        if entries_by_date.get(date_str, 0) >= 3:
            return None

        # Moderate momentum calculation
        momentum_score = self._calculate_moderate_momentum(prices, volumes, highs, lows, i)
        ...
        # Volume confirmation
        volume_analysis = self._analyze_moderate_volume(volumes, i)
        if volume_analysis["volume_ratio"] < 1.2:  # 1.2x average volume minimum
            return None

        # Technical analysis
        tech_analysis = self._moderate_technical_analysis(prices, highs, lows, i)
        
        # Moderate entry conditions
        if momentum_score > 0:  # Long entry
            if (tech_analysis["rsi"] >= 30.0 and tech_analysis["rsi"] <= 80.0 and  # Wider RSI range
                ...
                tech_analysis["volatility"] >= 0.005):  # Minimum volatility
                
                # Additional moderate filters
                if self._moderate_entry_filters(prices, volumes, i, is_long=True):
                    return {
                        "action": "buy_to_open",
                        "reason": f"MODERATE Momentum long: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x",
            ...
            if (tech_analysis["rsi"] >= 40.0 and tech_analysis["rsi"] <= 90.0 and  # Wider RSI range
                tech_analysis["trend_strength"] <= -0.2 and  # Moderate downtrend
                tech_analysis["volatility"] >= 0.005):  # Minimum volatility
                
                # Additional moderate filters
                if self._moderate_entry_filters(prices, volumes, i, is_long=False):
                    return {
                        "action": "sell_to_open",
                        "reason": f"MODERATE Momentum short: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x",
                        "confidence": min(abs(momentum_score) / 2.0, 1.0),
                        "technical_indicators": {
                            "momentum_score": momentum_score,
                            "rsi": tech_analysis["rsi"],
    ...
    def _check_moderate_exit(
        ...
        """Check for moderate exit conditions"""
        ...
        # Time-based exit (10-30 minutes for moderate trades)
        ...
        # Profit target (1.5% profit for moderate trades)
        if pnl_pct >= 1.5:
            exit_action = "sell_to_close" if direction == "long" else "buy_to_close"
            return {
                "action": exit_action,
        ...
        # Stop loss (1% loss for moderate trades)
        if pnl_pct <= -1.0:
            exit_action = "sell_to_close" if direction == "long" else "buy_to_close"
            return {
                "action": exit_action,
    ...
    def _calculate_moderate_momentum(
        self, 
        prices: List[float], 
        volumes: List[float], 
        highs: List[float], 
        ...
        """Moderate momentum calculation (balanced)"""
        if i < 10:
            return 0.0
        ...
        # Volatility boost (moderate)
        recent_volatility = (max(prices[max(0, i-5):i+1]) - min(prices[max(0, i-5):i+1])) / prices[i]
        volatility_boost = 1.0 + min(recent_volatility * 5, 0.8)  # Max 1.8x boost
    ...
    def _analyze_moderate_volume(self, volumes: List[float], i: int) -> Dict[str, float]:
        """Analyze volume quality (moderate)"""
    ...
    def _moderate_technical_analysis(self, prices: List[float], highs: List[float], lows: List[float], i: int) -> Dict[str, float]:
        """Moderate technical analysis"""
        if i < 15:
            return {"rsi": 50.0, "trend_strength": 0.0, "price_position": 0.5, "volatility": 0.005}
        
        # RSI calculation
        rsi = self._moderate_rsi(prices[max(0, i-14):i+1])
        ...
        # Weighted trend strength (moderate)
    ...
    def _moderate_rsi(self, prices: List[float], period: int = 14) -> float:
        """Moderate RSI calculation"""
    ...
    def _moderate_entry_filters(self, prices: List[float], volumes: List[float], i: int, is_long: bool) -> bool:
        """Moderate entry filters (balanced)"""
        if i < 8:
            return False
        
        # Price stability check
        recent_prices = prices[max(0, i-6):i+1]
        ...
        if volatility_ratio < 0.005 or volatility_ratio > 0.20:
            return False
        ...
        if volume_consistency < 0.15:  # Some consistency required
            return False
        ...
        if is_long:
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            return momentum_2 > -0.1 and momentum_5 > 0.0  # Moderate positive momentum
        else:
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            return momentum_2 < 0.1 and momentum_5 < 0.0  # Moderate negative momentum
    ...
    async def backtest_indicators(
        ...
        """Run moderate backtest"""
        print(f"Starting MODERATE backtest from {start_date} to {end_date}")
        print(f"Tickers: {', '.join(tickers)}")
        print(f"Indicators: {', '.join(indicators)}")
        print("⚖️  FOCUS: Moderate profitable trades (balanced quality/quantity)")
        ...
        for ticker in tickers:
            ...
            if "Momentum" in indicators:
                momentum_actions = self.simulate_moderate_momentum_trades(ticker, bars)
                print(f"  Moderate Momentum: {len(momentum_actions)} actions")
    ...
    def save_to_csv(self, actions: List[TradeAction], filename: str):
        ...
        print(f"Saved {len(actions)} MODERATE actions to {filename}")


async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Moderate profitable strategy backtest")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="Ticker symbols (if empty, uses moderate stocks)",
    )
    parser.add_argument(
        "--indicators",
        nargs="+",
    ...
    parser.add_argument(
        "--output",
        default="moderate_profitable_trading_2021_2026.csv",
    ...
    # Use moderate stocks if no tickers provided
    if not args.tickers:
        engine = ModerateProfitableStrategy(api_key, secret_key)
        tickers = engine.MODERATE_STOCKS
        print(f"Using moderate stocks: {', '.join(tickers)}")
    else:
    ...
    # Print summary
    print("\n⚖️  MODERATE PROFITABLE STRATEGY BACKTEST COMPLETED!")
    print(f"Total actions generated: {len(actions)}")
    print("⚖️  Goal: Moderate profitable trades (balanced quality/quantity)")
    ...
    if actions:
        ...
        print("\n⚠️  No moderate actions generated - try different parameters")
