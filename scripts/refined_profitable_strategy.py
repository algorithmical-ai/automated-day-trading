#!/usr/bin/env python3
"""
Refined Profitable Trading Strategy - Few High Quality Trades

This script implements a refined profitable trading strategy that:
1. Generates very few but high quality profitable trades
2. Creates complete trade pairs (entry + exit)
3. Outputs each action as separate CSV rows
4. Prioritizes quality over quantity with strict filters
"""
...
class RefinedProfitableStrategy:
    ...
    # Use only the best performing stocks from the working strategy
    REFINED_STOCKS = [
        "AAPL", "TSLA", "NVDA", "AMD", "META", "MSFT"  # Top 6 momentum stocks
    ]

    def simulate_refined_momentum_trades(
        self,
        ticker: str,
        bars: List[Dict[str, Any]],
    ) -> List[TradeAction]:
        """Simulate refined momentum trades - very few high quality"""
        actions = []

        if len(bars) < 30:
            return actions

        print(f"Simulating Refined Momentum for {ticker} with {len(bars)} bars")
        ...
        # Track active trades and very strict filters
        active_trades = {}
        entries_by_date = {}
        trade_counter = 0
        ...
        for i in range(25, len(bars)):
            ...
            # Very strict entry conditions for high quality
            entry_signal = self._check_refined_entry(
                ticker, prices, volumes, highs, lows, i, date_str, entries_by_date
            )

            if entry_signal and len(active_trades) < 1:  # Max 1 active trade
            ...
            for trade_id, trade_info in active_trades.items():
                exit_signal = self._check_refined_exit(
    ...
    def _check_refined_entry(
        ...
        """Check for refined entry conditions (very strict for high quality)"""
        
        # Max 1 trade per day (very strict)
        if entries_by_date.get(date_str, 0) >= 1:
            return None

        # Refined momentum calculation (very strict)
        momentum_score = self._calculate_refined_momentum(prices, volumes, highs, lows, i)
        
        # Very strict momentum threshold (only strong signals)
        if abs(momentum_score) < 0.8:  # 0.8% minimum momentum
            return None

        # Volume confirmation (very strict)
        volume_analysis = self._analyze_refined_volume(volumes, i)
        if volume_analysis["volume_ratio"] < 2.0:  # 2x average volume minimum
            return None

        # Technical analysis
        tech_analysis = self._refined_technical_analysis(prices, highs, lows, i)
        
        # Very strict entry conditions
        if momentum_score > 0:  # Long entry
            if (tech_analysis["rsi"] >= 35.0 and tech_analysis["rsi"] <= 75.0 and  # Narrow RSI range
                tech_analysis["trend_strength"] >= 0.4 and  # Strong uptrend
                tech_analysis["volatility"] >= 0.01 and  # Minimum volatility
                tech_analysis["price_position"] >= 0.3 and tech_analysis["price_position"] <= 0.8):  # Not at extremes
                
                # Additional refined filters
                if self._refined_entry_filters(prices, volumes, i, is_long=True):
                    return {
                        "action": "buy_to_open",
                        "reason": f"REFINED Momentum long: score={momentum_score:.3f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.3f}, vol={volume_analysis['volume_ratio']:.2f}x",
                        "confidence": min(abs(momentum_score) / 1.2, 1.0),
            ...
            if (tech_analysis["rsi"] >= 45.0 and tech_analysis["rsi"] <= 85.0 and  # Narrow RSI range
                tech_analysis["trend_strength"] <= -0.4 and  # Strong downtrend
                tech_analysis["volatility"] >= 0.01 and  # Minimum volatility
                tech_analysis["price_position"] >= 0.2 and tech_analysis["price_position"] <= 0.7):  # Not at extremes
                
                # Additional refined filters
                if self._refined_entry_filters(prices, volumes, i, is_long=False):
                    return {
                        "action": "sell_to_open",
                        "reason": f"REFINED Momentum short: score={momentum_score:.3f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.3f}, vol={volume_analysis['volume_ratio']:.2f}x",
                        "confidence": min(abs(momentum_score) / 1.2, 1.0),
                        "technical_indicators": {
                            "momentum_score": momentum_score,
                            "rsi": tech_analysis["rsi"],
    ...
    def _check_refined_exit(
        ...
        """Check for refined exit conditions (optimized for profitability)"""
        ...
        # Time-based exit (15-45 minutes for refined trades)
        if time_held >= 45:  # Max 45 minutes
        ...
        # Profit target (3% profit for refined trades)
        if pnl_pct >= 3.0:
            exit_action = "sell_to_close" if direction == "long" else "buy_to_close"
            return {
                "action": exit_action,
        ...
        # Stop loss (1% loss for refined trades)
        if pnl_pct <= -1.0:
            exit_action = "sell_to_close" if direction == "long" else "buy_to_close"
            return {
                "action": exit_action,
        ...
        # Momentum reversal exit (more sensitive for refined trades)
        momentum_score = self._calculate_refined_momentum(prices, volumes, highs, lows, i)
        if direction == "long" and momentum_score < -0.4:
            exit_action = "sell_to_close"
            return {
                "action": exit_action,
        ...
        elif direction == "short" and momentum_score > 0.4:
            exit_action = "buy_to_close"
            return {
                "action": exit_action,
                "reason": f"Momentum reversal, P&L: {pnl_pct:.2f}%",
    ...
    def _calculate_refined_momentum(
        self, 
        prices: List[float], 
        volumes: List[float], 
        highs: List[float], 
        ...
        # Weighted momentum (refined)
        momentum_score = (momentum_3 * 0.4 + momentum_5 * 0.4 + momentum_15 * 0.2)
        
        # Volume-weighted momentum (very strict)
        recent_volume = sum(volumes[max(0, i-5):i+1]) / 5
        avg_volume_10 = sum(volumes[max(0, i-15):i-5]) / 10 if i > 15 else recent_volume
        avg_volume_20 = sum(volumes[max(0, i-25):i-10]) / 15 if i > 25 else recent_volume
        volume_weight = min(recent_volume / max(avg_volume_10, avg_volume_20), 2.5) if max(avg_volume_10, avg_volume_20) > 0 else 1.0
        ...
        # Volatility boost (refined)
        recent_volatility = (max(prices[max(0, i-10):i+1]) - min(prices[max(0, i-10):i+1])) / prices[i]
        volatility_boost = 1.0 + min(recent_volatility * 8, 1.5)  # Max 2.5x boost
        
        # Final momentum score (refined)
        final_momentum = momentum_score * volume_weight * (0.6 + range_position * 0.4) * volatility_boost
        
        return final_momentum

    def _analyze_refined_volume(self, volumes: List[float], i: int) -> Dict[str, float]:
        """Analyze volume quality (very strict)"""
        ...
        # Volume trend (refined)
        volume_trend = (volumes[i] - volumes[max(0, i-10)]) / volumes[max(0, i-10)] if i > 10 and volumes[max(0, i-10)] > 0 else 0
    ...
    def _refined_technical_analysis(self, prices: List[float], highs: List[float], lows: List[float], i: int) -> Dict[str, float]:
        """Refined technical analysis (very strict)"""
        if i < 25:
            return {"rsi": 50.0, "trend_strength": 0.0, "price_position": 0.5, "volatility": 0.01}
        
        # RSI calculation
        rsi = self._refined_rsi(prices[max(0, i-20):i+1])
        
        # Trend strength calculation (refined)
        ...
        # Weighted trend strength (refined)
        trend_strength = (short_trend * 0.4 + medium_trend * 0.4 + long_trend * 0.2) / 3.0
    ...
    def _refined_rsi(self, prices: List[float], period: int = 20) -> float:
        """Refined RSI calculation (longer period)"""
    ...
    def _refined_entry_filters(self, prices: List[float], volumes: List[float], i: int, is_long: bool) -> bool:
        """Refined entry filters (very strict)"""
        if i < 15:
            return False
        
        # Price stability check (refined)
        ...
        # Refined volatility range
        if volatility_ratio < 0.01 or volatility_ratio > 0.15:
            return False
        
        # Volume consistency check (refined)
        ...
        if volume_consistency < 0.25:  # Higher consistency required
            return False
        
        # Price momentum consistency (refined)
        if is_long:
            momentum_3 = (prices[i] - prices[i-3]) / prices[i-3] * 100 if i >= 3 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            momentum_15 = (prices[i] - prices[i-15]) / prices[i-15] * 100 if i >= 15 else 0
            return momentum_3 > 0.1 and momentum_5 > 0.2 and momentum_15 > 0.3  # Strong positive momentum
        else:
            momentum_3 = (prices[i] - prices[i-3]) / prices[i-3] * 100 if i >= 3 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            momentum_15 = (prices[i] - prices[i-15]) / prices[i-15] * 100 if i >= 15 else 0
            return momentum_3 < -0.1 and momentum_5 < -0.2 and momentum_15 < -0.3  # Strong negative momentum
    ...
    async def backtest_indicators(
        ...
        print(f"Starting REFINED backtest from {start_date} to {end_date}")
        print(f"Tickers: {', '.join(tickers)}")
        print(f"Indicators: {', '.join(indicators)}")
        print("🎯 FOCUS: Refined profitable trades (few high quality)")
        ...
        for ticker in tickers:
            ...
            if "Momentum" in indicators:
                momentum_actions = self.simulate_refined_momentum_trades(ticker, bars)
                print(f"  Refined Momentum: {len(momentum_actions)} actions")
    ...
    def save_to_csv(self, actions: List[TradeAction], filename: str):
        ...
        print(f"Saved {len(actions)} REFINED actions to {filename}")


async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Refined profitable strategy backtest")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="Ticker symbols (if empty, uses refined stocks)",
    )
    parser.add_argument(
        "--indicators",
        nargs="+",
    ...
    parser.add_argument(
        "--output",
        default="refined_profitable_trading_2021_2026.csv",
    ...
    # Use refined stocks if no tickers provided
    if not args.tickers:
        engine = RefinedProfitableStrategy(api_key, secret_key)
        tickers = engine.REFINED_STOCKS
        print(f"Using refined stocks: {', '.join(tickers)}")
    else:
    ...
    # Print summary
    print("\n🎯 REFINED PROFITABLE STRATEGY BACKTEST COMPLETED!")
    print(f"Total actions generated: {len(actions)}")
    print("🎯 Goal: Refined profitable trades (few high quality)")
    ...
    if actions:
        ...
        print("\n⚠️  No refined actions generated - try different parameters")
