#!/usr/bin/env python3
"""
Optimized Quality Trading Strategy - Balanced High Quality Trades

This script implements an optimized quality trading strategy that:
...
4. Balances quality with reasonable quantity
"""
...
class OptimizedQualityStrategy:
    """Optimized quality trading strategy - balanced high quality trades"""

    def __init__(self, api_key: str, secret_key: str):
        self.alpaca_client = AlpacaHistoricalClient(api_key, secret_key)

    # Use top performing stocks from working strategy
    OPTIMIZED_STOCKS = [
        "AAPL", "TSLA", "NVDA", "AMD", "META", "MSFT", "AMZN", "GOOGL"
    ]

    def simulate_optimized_momentum_trades(
        self,
        ticker: str,
        bars: List[Dict[str, Any]],
    ) -> List[TradeAction]:
        """Simulate optimized momentum trades - balanced quality"""
        actions = []

        if len(bars) < 20:
            return actions

        print(f"Simulating Optimized Momentum for {ticker} with {len(bars)} bars")
        ...
        # Track active trades and optimized filters
        active_trades = {}
        entries_by_date = {}
        trade_counter = 0
        ...
        for i in range(20, len(bars)):
            ...
            # Optimized entry conditions (balanced)
            entry_signal = self._check_optimized_entry(
                ticker, prices, volumes, highs, lows, i, date_str, entries_by_date
            )

            if entry_signal and len(active_trades) < 2:  # Max 2 active trades
            ...
            for trade_id, trade_info in active_trades.items():
                exit_signal = self._check_optimized_exit(
    ...
    def _check_optimized_entry(
        ...
        """Check for optimized entry conditions (balanced quality)"""
        
        # Max 2 trades per day (balanced)
        if entries_by_date.get(date_str, 0) >= 2:
            return None

        # Optimized momentum calculation (balanced)
        momentum_score = self._calculate_optimized_momentum(prices, volumes, highs, lows, i)
        
        # Optimized momentum threshold (balanced)
        if abs(momentum_score) < 0.4:  # 0.4% minimum momentum
            return None

        # Volume confirmation (balanced)
        volume_analysis = self._analyze_optimized_volume(volumes, i)
        if volume_analysis["volume_ratio"] < 1.3:  # 1.3x average volume minimum
            return None
        ...
        if momentum_score > 0:  # Long entry
            if (tech_analysis["rsi"] >= 35.0 and tech_analysis["rsi"] <= 80.0 and  # Balanced RSI range
                tech_analysis["trend_strength"] >= 0.2 and  # Moderate uptrend
                tech_analysis["volatility"] >= 0.008 and  # Minimum volatility
                tech_analysis["price_position"] >= 0.2 and tech_analysis["price_position"] <= 0.85):  # Not at extremes
                
                # Additional optimized filters
                if self._optimized_entry_filters(prices, volumes, i, is_long=True):
                    return {
                        "action": "buy_to_open",
                        "reason": f"OPTIMIZED Momentum long: score={momentum_score:.3f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.3f}, vol={volume_analysis['volume_ratio']:.2f}x",
            ...
            if (tech_analysis["rsi"] >= 40.0 and tech_analysis["rsi"] <= 85.0 and  # Balanced RSI range
                tech_analysis["trend_strength"] <= -0.2 and  # Moderate downtrend
                tech_analysis["volatility"] >= 0.008 and  # Minimum volatility
                tech_analysis["price_position"] >= 0.15 and tech_analysis["price_position"] <= 0.8):  # Not at extremes
                
                # Additional optimized filters
                if self._optimized_entry_filters(prices, volumes, i, is_long=False):
                    return {
                        "action": "sell_to_open",
                        "reason": f"OPTIMIZED Momentum short: score={momentum_score:.3f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.3f}, vol={volume_analysis['volume_ratio']:.2f}x",
                        "confidence": min(abs(momentum_score) / 1.5, 1.0),
                        "technical_indicators": {
                            "momentum_score": momentum_score,
                            "rsi": tech_analysis["rsi"],
    ...
    def _check_optimized_exit(
        ...
        """Check for optimized exit conditions (balanced)"""
        ...
        # Time-based exit (10-40 minutes for optimized trades)
        if time_held >= 40:  # Max 40 minutes
        ...
        # Profit target (2.5% profit for optimized trades)
        if pnl_pct >= 2.5:
            exit_action = "sell_to_close" if direction == "long" else "buy_to_close"
            return {
                "action": exit_action,
        ...
        # Stop loss (1.2% loss for optimized trades)
        if pnl_pct <= -1.2:
            exit_action = "sell_to_close" if direction == "long" else "buy_to_close"
            return {
                "action": exit_action,
        ...
        # Momentum reversal exit (balanced)
        momentum_score = self._calculate_optimized_momentum(prices, volumes, highs, lows, i)
        if direction == "long" and momentum_score < -0.3:
            exit_action = "sell_to_close"
            return {
                "action": exit_action,
        ...
        elif direction == "short" and momentum_score > 0.3:
            exit_action = "buy_to_close"
            return {
                "action": exit_action,
                "reason": f"Momentum reversal, P&L: {pnl_pct:.2f}%",
    ...
    def _calculate_optimized_momentum(
        ...
        # Weighted momentum (optimized)
        momentum_score = (momentum_2 * 0.4 + momentum_5 * 0.4 + momentum_10 * 0.2)
        ...
        volume_weight = min(recent_volume / max(avg_volume_10, avg_volume_20), 2.2) if max(avg_volume_10, avg_volume_20) > 0 else 1.0
        ...
        # Volatility boost (optimized)
        recent_volatility = (max(prices[max(0, i-8):i+1]) - min(prices[max(0, i-8):i+1])) / prices[i]
        volatility_boost = 1.0 + min(recent_volatility * 6, 1.2)  # Max 2.2x boost
        
        # Final momentum score (optimized)
        final_momentum = momentum_score * volume_weight * (0.65 + range_position * 0.35) * volatility_boost
        
        return final_momentum

    def _analyze_optimized_volume(self, volumes: List[float], i: int) -> Dict[str, float]:
    ...
    def _optimized_technical_analysis(self, prices: List[float], highs: List[float], lows: List[float], i: int) -> Dict[str, float]:
        """Optimized technical analysis (balanced)"""
        if i < 20:
            return {"rsi": 50.0, "trend_strength": 0.0, "price_position": 0.5, "volatility": 0.008}
        
        # RSI calculation
        rsi = self._optimized_rsi(prices[max(0, i-18):i+1])
        
        # Trend strength calculation (optimized)
        ...
        # Weighted trend strength (optimized)
    ...
    def _optimized_rsi(self, prices: List[float], period: int = 18) -> float:
        """Optimized RSI calculation (balanced period)"""
    ...
    def _optimized_entry_filters(self, prices: List[float], volumes: List[float], i: int, is_long: bool) -> bool:
        """Optimized entry filters (balanced)"""
        if i < 12:
            return False
        
        # Price stability check (optimized)
        ...
        # Optimized volatility range
        if volatility_ratio < 0.008 or volatility_ratio > 0.18:
            return False
        
        # Volume consistency check (optimized)
        ...
        if volume_consistency < 0.2:  # Reasonable consistency required
            return False
        
        # Price momentum consistency (optimized)
        if is_long:
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            momentum_10 = (prices[i] - prices[i-10]) / prices[i-10] * 100 if i >= 10 else 0
            return momentum_2 > 0.05 and momentum_5 > 0.1 and momentum_10 > 0.15  # Positive momentum
        else:
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            momentum_10 = (prices[i] - prices[i-10]) / prices[i-10] * 100 if i >= 10 else 0
            return momentum_2 < -0.05 and momentum_5 < -0.1 and momentum_10 < -0.15  # Negative momentum
    ...
    async def backtest_indicators(
        ...
        print("⚡ FOCUS: Optimized quality trades (balanced)")
        ...
        for ticker in tickers:
            ...
            if "Momentum" in indicators:
                momentum_actions = self.simulate_optimized_momentum_trades(ticker, bars)
...
async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Optimized quality strategy backtest")
    ...
    parser.add_argument(
        "--output",
        default="optimized_quality_trading_2021_2026.csv",
    ...
    if not args.tickers:
        engine = OptimizedQualityStrategy(api_key, secret_key)
        tickers = engine.OPTIMIZED_STOCKS
    ...
    # Print summary
    print("\n⚡ OPTIMIZED QUALITY STRATEGY BACKTEST COMPLETED!")
    print(f"Total actions generated: {len(actions)}")
    print("⚡ Goal: Optimized quality trades (balanced)")
