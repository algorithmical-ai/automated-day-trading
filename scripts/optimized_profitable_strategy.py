#!/usr/bin/env python3
"""
Optimized Profitable Trading Strategy Implementation

This script implements an optimized profitable trading strategy that generates
fewer but higher quality trades. Uses the working approach from complete trade
pairs but with better filters for quality and profitability.
"""

import asyncio
import argparse
import os
from datetime import datetime, timedelta
from typing import Dict, List, Any
from dataclasses import dataclass
import requests
from dotenv import load_dotenv


@dataclass
class TradeAction:
    """Represents a complete trade action (entry or exit)"""
    timestamp: str
    ticker: str
    action: str  # buy_to_open, sell_to_close, sell_to_open, buy_to_close
    price: float
    indicator: str
    trade_id: str
    signal_type: str  # entry or exit
    reason: str
    confidence: float = 0.0
    technical_indicators: Dict[str, Any] = None
    pnl: float = 0.0
    time_held: int = 0  # minutes held


class AlpacaHistoricalClient:
    """Client for fetching historical data from Alpaca"""

    def __init__(self, api_key: str, secret_key: str):
        self.api_key = api_key
        self.secret_key = secret_key
        self.base_url = "https://data.alpaca.markets/v2/stocks"
        self.headers = {
            "APCA-API-KEY-ID": api_key,
            "APCA-API-SECRET-KEY": secret_key,
        }

    async def fetch_bars(
        self,
        symbol: str,
        start: str,
        end: str,
        timeframe: str = "1Min",
    ) -> List[Dict[str, Any]]:
        """Fetch historical bars for a symbol"""
        url = f"{self.base_url}/bars"

        # Convert datetime strings to proper format
        if "T" in start:
            start_dt = datetime.fromisoformat(start.replace("Z", "+00:00"))
            start_str = start_dt.strftime("%Y-%m-%dT%H:%M:%SZ")
        else:
            start_str = f"{start}T00:00:00Z"

        if "T" in end:
            end_dt = datetime.fromisoformat(end.replace("Z", "+00:00"))
            end_str = end_dt.strftime("%Y-%m-%dT%H:%M:%SZ")
        else:
            end_str = f"{end}T23:59:59Z"

        params = {
            "symbols": symbol,
            "timeframe": timeframe,
            "start": start_str,
            "end": end_str,
            "adjustment": "raw",
            "feed": "sip",
        }

        try:
            response = requests.get(url, headers=self.headers, params=params, timeout=30)
            response.raise_for_status()
            data = response.json()

            if symbol in data.get("bars", {}):
                return data["bars"][symbol]
            return []

        except Exception as e:
            print(f"Error fetching bars for {symbol}: {e}")
            return []

    async def fetch_bars_batch(
        self,
        symbols: List[str],
        start_date: str,
        end_date: str,
        batch_days: int = 7,
    ) -> Dict[str, List[Dict[str, Any]]]:
        """Fetch bars in batches to handle large date ranges"""
        all_bars = {symbol: [] for symbol in symbols}

        # Parse dates
        if "T" in start_date:
            start_dt = datetime.fromisoformat(start_date.replace("Z", "+00:00"))
        else:
            start_dt = datetime.strptime(start_date, "%Y-%m-%d")

        if "T" in end_date:
            end_dt = datetime.fromisoformat(end_date.replace("Z", "+00:00"))
        else:
            end_dt = datetime.strptime(end_date, "%Y-%m-%d")

        # Process in batches
        current_start = start_dt
        batch_num = 1

        while current_start < end_dt:
            batch_end = min(current_start + timedelta(days=batch_days), end_dt)

            print(
                f"Fetching batch {batch_num}: {current_start.strftime('%Y-%m-%d')} to {batch_end.strftime('%Y-%m-%d')}"
            )

            # Fetch for all symbols in this batch
            tasks = []
            for symbol in symbols:
                task = self.fetch_bars(
                    symbol,
                    current_start.strftime("%Y-%m-%d"),
                    batch_end.strftime("%Y-%m-%d"),
                )
                tasks.append(task)

            batch_results = await asyncio.gather(*tasks, return_exceptions=True)

            # Process results
            for i, result in enumerate(batch_results):
                symbol = symbols[i]
                if isinstance(result, Exception):
                    print(f"Error fetching {symbol}: {result}")
                elif result:
                    all_bars[symbol].extend(result)

            current_start = batch_end
            batch_num += 1

            # Rate limiting
            await asyncio.sleep(1)

        # Sort bars by timestamp
        for symbol in symbols:
            all_bars[symbol].sort(key=lambda x: x["t"])

        return all_bars


class OptimizedProfitableStrategy:
    """Optimized profitable trading strategy implementation"""

    def __init__(self, api_key: str, secret_key: str):
        self.alpaca_client = AlpacaHistoricalClient(api_key, secret_key)

    # High-quality stocks for optimized strategy
    OPTIMIZED_STOCKS = [
        "AAPL", "TSLA", "NVDA", "AMD", "META", "NFLX", "AMZN", "GOOGL", "MSFT", "SPY"
    ]

    # Penny stocks for optimized strategy
    PENNY_STOCKS = [
        "AMC", "GME", "BB", "NOK", "SNDL", "BNGO", "MVIS", "SPCE", "BITF", "RIOT", "MARA", "HUT"
    ]

    def simulate_optimized_momentum(
        self,
        ticker: str,
        bars: List[Dict[str, Any]],
        indicator_type: str = "Momentum"
    ) -> List[TradeAction]:
        """Optimized momentum strategy with quality filters"""
        actions = []

        if len(bars) < 50:
            return actions

        print(f"Simulating Optimized {indicator_type} for {ticker} with {len(bars)} bars")

        # Extract price and volume data
        prices = [float(bar.get("c", 0)) for bar in bars]
        volumes = [float(bar.get("v", 0)) for bar in bars]
        highs = [float(bar.get("h", 0)) for bar in bars]
        lows = [float(bar.get("l", 0)) for bar in bars]
        timestamps = [bar.get("t", "") for bar in bars]

        # Track entries and exits
        open_positions = {}
        trade_counter = 1
        
        # Track entries per day for quality control
        entries_by_date = {}
        
        # Set different limits based on indicator type
        daily_entry_limit = 2 if indicator_type == "PennyStocks" else 1

        for i in range(50, len(bars)):
            current_bar = bars[i]
            timestamp = timestamps[i]
            price = prices[i]
            
            # Extract date from timestamp
            if "T" in timestamp:
                date_str = timestamp.split("T")[0]
            else:
                date_str = timestamp[:10]

            # Skip if already reached daily entry limit
            if entries_by_date.get(date_str, 0) >= daily_entry_limit:
                continue

            # Check for exit signals first
            for trade_id, position in list(open_positions.items()):
                exit_action = self._check_exit_optimized(
                    ticker, timestamp, price, position, i, prices, volumes, highs, lows
                )
                if exit_action:
                    actions.append(exit_action)
                    del open_positions[trade_id]

            # Only enter new positions if not too many open
            if len(open_positions) >= 3:  # Max 3 open positions at once
                continue

            # Optimized momentum calculation
            momentum_score = self._calculate_optimized_momentum(prices, volumes, highs, lows, i)
            
            # Higher threshold for quality (but not too high)
            if abs(momentum_score) >= 0.8:  # 0.8% minimum momentum
                # Volume confirmation
                volume_analysis = self._analyze_volume_optimized(volumes, i)
                
                if volume_analysis["volume_ratio"] >= 1.5:  # 1.5x average volume
                    # Technical analysis
                    tech_analysis = self._optimized_technical_analysis(prices, highs, lows, i)
                    
                    # Optimized entry conditions
                    if momentum_score > 0:  # Long entry
                        if (tech_analysis["rsi"] >= 35.0 and tech_analysis["rsi"] <= 75.0 and
                            tech_analysis["trend_strength"] >= 0.3 and
                            tech_analysis["volatility"] >= 0.008):
                            
                            # Quality filters
                            if self._optimized_quality_filters(prices, volumes, i, is_long=True):
                                action = "buy_to_open"
                                reason = f"OPTIMIZED {indicator_type} long: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x"
                                confidence = min(abs(momentum_score) / 2.0, 1.0)
                                
                                trade_id = f"{ticker}_{trade_counter}"
                                trade_counter += 1
                                
                                trade_action = TradeAction(
                                    timestamp=timestamp,
                                    ticker=ticker,
                                    action=action,
                                    price=price,
                                    indicator=f"Optimized {indicator_type}",
                                    trade_id=trade_id,
                                    signal_type="entry",
                                    reason=reason,
                                    confidence=confidence,
                                    technical_indicators={
                                        "momentum_score": momentum_score,
                                        "rsi": tech_analysis["rsi"],
                                        "trend_strength": tech_analysis["trend_strength"],
                                        "volume_ratio": volume_analysis["volume_ratio"],
                                        "price_position": tech_analysis["price_position"],
                                        "volatility": tech_analysis["volatility"],
                                    }
                                )
                                actions.append(trade_action)
                                open_positions[trade_id] = {
                                    "entry_price": price,
                                    "entry_time": timestamp,
                                    "entry_index": i,
                                    "direction": "long",
                                    "indicator": f"Optimized {indicator_type}"
                                }
                                entries_by_date[date_str] = entries_by_date.get(date_str, 0) + 1

                    elif momentum_score < 0:  # Short entry
                        if (tech_analysis["rsi"] >= 50.0 and tech_analysis["rsi"] <= 85.0 and
                            tech_analysis["trend_strength"] <= -0.3 and
                            tech_analysis["volatility"] >= 0.008):
                            
                            # Quality filters
                            if self._optimized_quality_filters(prices, volumes, i, is_long=False):
                                action = "sell_to_open"
                                reason = f"OPTIMIZED {indicator_type} short: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x"
                                confidence = min(abs(momentum_score) / 2.0, 1.0)
                                
                                trade_id = f"{ticker}_{trade_counter}"
                                trade_counter += 1
                                
                                trade_action = TradeAction(
                                    timestamp=timestamp,
                                    ticker=ticker,
                                    action=action,
                                    price=price,
                                    indicator=f"Optimized {indicator_type}",
                                    trade_id=trade_id,
                                    signal_type="entry",
                                    reason=reason,
                                    confidence=confidence,
                                    technical_indicators={
                                        "momentum_score": momentum_score,
                                        "rsi": tech_analysis["rsi"],
                                        "trend_strength": tech_analysis["trend_strength"],
                                        "volume_ratio": volume_analysis["volume_ratio"],
                                        "price_position": tech_analysis["price_position"],
                                        "volatility": tech_analysis["volatility"],
                                    }
                                )
                                actions.append(trade_action)
                                open_positions[trade_id] = {
                                    "entry_price": price,
                                    "entry_time": timestamp,
                                    "entry_index": i,
                                    "direction": "short",
                                    "indicator": f"Optimized {indicator_type}"
                                }
                                entries_by_date[date_str] = entries_by_date.get(date_str, 0) + 1

        # Close any remaining open positions at the end
        for trade_id, position in open_positions.items():
            exit_action = TradeAction(
                timestamp=timestamps[-1],
                ticker=ticker,
                action="sell_to_close" if position["direction"] == "long" else "buy_to_close",
                price=prices[-1],
                indicator=position["indicator"],
                trade_id=trade_id,
                signal_type="exit",
                reason="End of data - force close",
                confidence=0.5,
                pnl=(prices[-1] - position["entry_price"]) if position["direction"] == "long" else (position["entry_price"] - prices[-1]),
                time_held=len(timestamps) - position["entry_index"]
            )
            actions.append(exit_action)

        return actions

    def _calculate_optimized_momentum(
        self, 
        prices: List[float], 
        volumes: List[float], 
        highs: List[float], 
        lows: List[float], 
        i: int
    ) -> float:
        """Optimized momentum calculation"""
        if i < 20:
            return 0.0

        # Multiple timeframe momentum
        momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
        momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
        momentum_10 = (prices[i] - prices[i-10]) / prices[i-10] * 100 if i >= 10 else 0
        momentum_20 = (prices[i] - prices[i-20]) / prices[i-20] * 100 if i >= 20 else 0
        
        # Weighted momentum (optimized for quality)
        momentum_score = (momentum_2 * 0.4 + momentum_5 * 0.3 + momentum_10 * 0.2 + momentum_20 * 0.1)
        
        # Volume-weighted momentum
        recent_volume = sum(volumes[max(0, i-3):i+1]) / 3
        avg_volume = sum(volumes[max(0, i-15):i-3]) / 12 if i > 15 else recent_volume
        volume_weight = min(recent_volume / avg_volume, 2.0) if avg_volume > 0 else 1.0
        
        # Price range momentum
        recent_high = max(highs[max(0, i-8):i])
        recent_low = min(lows[max(0, i-8):i])
        range_position = (prices[i] - recent_low) / (recent_high - recent_low) if recent_high > recent_low else 0.5
        
        # Volatility boost (capped)
        recent_volatility = (max(prices[max(0, i-8):i+1]) - min(prices[max(0, i-8):i+1])) / prices[i]
        volatility_boost = 1.0 + min(recent_volatility * 3, 0.4)  # Max 1.4x boost
        
        # Final momentum score
        final_momentum = momentum_score * volume_weight * (0.7 + range_position * 0.3) * volatility_boost
        
        return final_momentum

    def _analyze_volume_optimized(self, volumes: List[float], i: int) -> Dict[str, float]:
        """Analyze volume quality (optimized)"""
        # Recent volume analysis
        recent_volume = sum(volumes[max(0, i-3):i+1]) / 3
        avg_volume_5 = sum(volumes[max(0, i-10):i-3]) / 7 if i > 10 else recent_volume
        avg_volume_20 = sum(volumes[max(0, i-25):i-5]) / 20 if i > 25 else recent_volume
        
        volume_ratio_5 = recent_volume / avg_volume_5 if avg_volume_5 > 0 else 1.0
        volume_ratio_20 = recent_volume / avg_volume_20 if avg_volume_20 > 0 else 1.0
        
        # Volume trend
        volume_trend = (volumes[i] - volumes[max(0, i-8)]) / volumes[max(0, i-8)] if i > 8 and volumes[max(0, i-8)] > 0 else 0
        
        return {
            "volume_ratio": max(volume_ratio_5, volume_ratio_20),
            "volume_trend": volume_trend,
            "recent_volume": recent_volume,
        }

    def _optimized_technical_analysis(self, prices: List[float], highs: List[float], lows: List[float], i: int) -> Dict[str, float]:
        """Optimized technical analysis"""
        if i < 20:
            return {"rsi": 50.0, "trend_strength": 0.0, "price_position": 0.5, "volatility": 0.008}
        
        # RSI calculation
        rsi = self._optimized_rsi(prices[max(0, i-14):i+1])
        
        # Trend strength calculation
        short_trend = (prices[i] - prices[i-3]) / prices[i-3] if i >= 3 else 0
        medium_trend = (prices[i] - prices[i-10]) / prices[i-10] if i >= 10 else 0
        long_trend = (prices[i] - prices[i-20]) / prices[i-20] if i >= 20 else 0
        
        # Weighted trend strength
        trend_strength = (short_trend * 0.5 + medium_trend * 0.3 + long_trend * 0.2)
        trend_strength = max(-1.0, min(1.0, trend_strength))
        
        # Price position in recent range
        recent_high = max(highs[max(0, i-10):i])
        recent_low = min(lows[max(0, i-10):i])
        price_position = (prices[i] - recent_low) / (recent_high - recent_low) if recent_high > recent_low else 0.5
        
        # Volatility
        recent_prices = prices[max(0, i-15):i+1]
        volatility = (max(recent_prices) - min(recent_prices)) / sum(recent_prices) * len(recent_prices) if recent_prices else 0.008
        
        return {
            "rsi": rsi,
            "trend_strength": trend_strength,
            "price_position": price_position,
            "volatility": volatility,
        }

    def _optimized_rsi(self, prices: List[float], period: int = 14) -> float:
        """Optimized RSI calculation"""
        if len(prices) < period + 1:
            return 50.0
        
        gains = []
        losses = []
        
        for i in range(1, len(prices)):
            change = prices[i] - prices[i - 1]
            if change > 0:
                gains.append(change)
                losses.append(0)
            else:
                gains.append(0)
                losses.append(abs(change))
        
        if len(gains) >= period:
            avg_gain = sum(gains[-period:]) / period
            avg_loss = sum(losses[-period:]) / period
        else:
            avg_gain = sum(gains) / len(gains) if gains else 0
            avg_loss = sum(losses) / len(losses) if losses else 0
        
        if avg_loss == 0:
            return 100.0
        else:
            rs = avg_gain / avg_loss
            return 100.0 - (100.0 / (1.0 + rs))

    def _optimized_quality_filters(self, prices: List[float], volumes: List[float], i: int, is_long: bool) -> bool:
        """Optimized quality filters"""
        if i < 15:
            return False
        
        # Price stability check
        recent_prices = prices[max(0, i-10):i+1]
        price_volatility = max(recent_prices) - min(recent_prices)
        avg_price = sum(recent_prices) / len(recent_prices)
        volatility_ratio = price_volatility / avg_price if avg_price > 0 else 1.0
        
        # Require reasonable volatility
        if volatility_ratio < 0.008 or volatility_ratio > 0.15:
            return False
        
        # Volume consistency check
        recent_volumes = volumes[max(0, i-5):i+1]
        volume_consistency = min(recent_volumes) / max(recent_volumes) if max(recent_volumes) > 0 else 0
        
        if volume_consistency < 0.2:
            return False
        
        # Price momentum consistency
        if is_long:
            # For long: recent momentum should be positive
            momentum_3 = (prices[i] - prices[i-3]) / prices[i-3] * 100 if i >= 3 else 0
            momentum_8 = (prices[i] - prices[i-8]) / prices[i-8] * 100 if i >= 8 else 0
            return momentum_3 > -0.2 and momentum_8 > -0.1
        else:
            # For short: recent momentum should be negative
            momentum_3 = (prices[i] - prices[i-3]) / prices[i-3] * 100 if i >= 3 else 0
            momentum_8 = (prices[i] - prices[i-8]) / prices[i-8] * 100 if i >= 8 else 0
            return momentum_3 < 0.2 and momentum_8 < 0.1
        
        return True

    def _check_exit_optimized(
        self, 
        ticker: str, 
        timestamp: str, 
        current_price: float, 
        position: Dict, 
        i: int,
        prices: List[float],
        volumes: List[float],
        highs: List[float],
        lows: List[float]
    ) -> TradeAction:
        """Check if position should be closed (optimized)"""
        entry_price = position["entry_price"]
        entry_time = position["entry_time"]
        entry_index = position["entry_index"]
        direction = position["direction"]
        
        # Calculate time held
        time_held = i - entry_index
        
        # Calculate P&L
        if direction == "long":
            pnl = current_price - entry_price
            pnl_percent = pnl / entry_price * 100
        else:
            pnl = entry_price - current_price
            pnl_percent = pnl / entry_price * 100
        
        # Exit conditions (optimized)
        # 1. Time-based exit (hold longer for better trades)
        if time_held >= 180:  # 3 hours max
            return TradeAction(
                timestamp=timestamp,
                ticker=ticker,
                action="sell_to_close" if direction == "long" else "buy_to_close",
                price=current_price,
                indicator=position["indicator"],
                trade_id="",  # Will be set by caller
                signal_type="exit",
                reason=f"Time exit: {time_held} min held",
                confidence=0.6,
                pnl=pnl,
                time_held=time_held
            )
        
        # 2. Profit target (higher for quality)
        if pnl_percent >= 2.0:  # 2% profit target
            return TradeAction(
                timestamp=timestamp,
                ticker=ticker,
                action="sell_to_close" if direction == "long" else "buy_to_close",
                price=current_price,
                indicator=position["indicator"],
                trade_id="",  # Will be set by caller
                signal_type="exit",
                reason=f"Profit target: {pnl_percent:.2f}%",
                confidence=0.9,
                pnl=pnl,
                time_held=time_held
            )
        
        # 3. Stop loss (tighter for quality)
        if pnl_percent <= -1.0:  # 1% stop loss
            return TradeAction(
                timestamp=timestamp,
                ticker=ticker,
                action="sell_to_close" if direction == "long" else "buy_to_close",
                price=current_price,
                indicator=position["indicator"],
                trade_id="",  # Will be set by caller
                signal_type="exit",
                reason=f"Stop loss: {pnl_percent:.2f}%",
                confidence=0.8,
                pnl=pnl,
                time_held=time_held
            )
        
        # 4. Momentum reversal exit
        if i >= 10:
            momentum_reversal = self._check_momentum_reversal_optimized(prices, volumes, highs, lows, i, direction)
            if momentum_reversal:
                return TradeAction(
                    timestamp=timestamp,
                    ticker=ticker,
                    action="sell_to_close" if direction == "long" else "buy_to_close",
                    price=current_price,
                    indicator=position["indicator"],
                    trade_id="",  # Will be set by caller
                    signal_type="exit",
                    reason="Momentum reversal",
                    confidence=0.7,
                    pnl=pnl,
                    time_held=time_held
                )
        
        return None

    def _check_momentum_reversal_optimized(self, prices: List[float], volumes: List[float], highs: List[float], lows: List[float], i: int, direction: str) -> bool:
        """Check for momentum reversal (optimized)"""
        if i < 10:
            return False
        
        # Recent momentum
        momentum_3 = (prices[i] - prices[i-3]) / prices[i-3] * 100 if i >= 3 else 0
        momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
        
        # Volume confirmation
        recent_volume = sum(volumes[max(0, i-3):i+1]) / 3
        avg_volume = sum(volumes[max(0, i-10):i-3]) / 7 if i > 10 else recent_volume
        volume_ratio = recent_volume / avg_volume if avg_volume > 0 else 1.0
        
        if direction == "long":
            # For long positions: check for bearish reversal
            return (momentum_3 < -0.3 and momentum_5 < -0.2 and volume_ratio > 1.3)
        else:
            # For short positions: check for bullish reversal
            return (momentum_3 > 0.3 and momentum_5 > 0.2 and volume_ratio > 1.3)

    async def backtest_indicators(
        self,
        tickers: List[str],
        start_date: str,
        end_date: str,
        indicators: List[str],
    ) -> List[TradeAction]:
        """Run optimized backtest"""
        print(f"Starting OPTIMIZED backtest from {start_date} to {end_date}")
        print(f"Tickers: {', '.join(tickers)}")
        print(f"Indicators: {', '.join(indicators)}")
        print("🎯 FOCUS: High quality profitable trades (fewer but better)")

        # Fetch historical data
        all_bars = await self.alpaca_client.fetch_bars_batch(
            symbols=tickers, start_date=start_date, end_date=end_date
        )

        all_actions = []

        for ticker in tickers:
            bars = all_bars.get(ticker, [])
            if not bars:
                print(f"No data found for {ticker}")
                continue

            print(f"Backtesting {ticker} with {len(bars)} bars...")

            if "Momentum" in indicators:
                momentum_actions = self.simulate_optimized_momentum(ticker, bars, "Momentum")
                print(f"  Optimized Momentum: {len(momentum_actions)} actions")
                all_actions.extend(momentum_actions)

            if "PennyStocks" in indicators:
                pennystocks_actions = self.simulate_optimized_momentum(ticker, bars, "PennyStocks")
                print(f"  Optimized PennyStocks: {len(pennystocks_actions)} actions")
                all_actions.extend(pennystocks_actions)

        return all_actions

    def save_to_csv(self, actions: List[TradeAction], filename: str):
        """Save actions to CSV file"""
        import csv

        with open(filename, "w", newline="") as csvfile:
            fieldnames = [
                "timestamp",
                "ticker",
                "action",
                "price",
                "indicator",
                "trade_id",
                "signal_type",
                "reason",
                "confidence",
                "momentum_score",
                "rsi",
                "trend_strength",
                "volume_ratio",
                "price_position",
                "volatility",
                "pnl",
                "time_held"
            ]
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()

            for action in actions:
                indicators = action.technical_indicators or {}
                
                row = {
                    "timestamp": action.timestamp,
                    "ticker": action.ticker,
                    "action": action.action,
                    "price": action.price,
                    "indicator": action.indicator,
                    "trade_id": action.trade_id,
                    "signal_type": action.signal_type,
                    "reason": action.reason,
                    "confidence": action.confidence,
                    "momentum_score": indicators.get("momentum_score", ""),
                    "rsi": indicators.get("rsi", ""),
                    "trend_strength": indicators.get("trend_strength", ""),
                    "volume_ratio": indicators.get("volume_ratio", ""),
                    "price_position": indicators.get("price_position", ""),
                    "volatility": indicators.get("volatility", ""),
                    "pnl": action.pnl,
                    "time_held": action.time_held
                }
                writer.writerow(row)

        print(f"Saved {len(actions)} OPTIMIZED actions to {filename}")


async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Optimized profitable strategy backtest")
    parser.add_argument("--start-date", required=True, help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", required=True, help="End date (YYYY-MM-DD)")
    parser.add_argument(
        "--tickers",
        nargs="+",
        default=[],
        help="Ticker symbols (if empty, uses optimized stocks)",
    )
    parser.add_argument(
        "--indicators",
        nargs="+",
        default=["Momentum", "PennyStocks"],
        help="Indicators to backtest",
    )
    parser.add_argument(
        "--output",
        default="optimized_profitable_trading_2021_2026.csv",
        help="Output CSV filename",
    )

    args = parser.parse_args()

    # Load environment variables
    load_dotenv()

    api_key = os.getenv("ALPACA_API_KEY")
    secret_key = os.getenv("ALPACA_SECRET_KEY")

    if not api_key or not secret_key:
        print("Error: ALPACA_API_KEY and ALPACA_SECRET_KEY must be set")
        return

    # Use optimized stocks if no tickers provided
    if not args.tickers:
        engine = OptimizedProfitableStrategy(api_key, secret_key)
        # Use both momentum and penny stocks
        tickers = engine.OPTIMIZED_STOCKS + engine.PENNY_STOCKS
        print(f"Using optimized stocks: {', '.join(tickers)}")
    else:
        tickers = args.tickers

    # Run backtest
    engine = OptimizedProfitableStrategy(api_key, secret_key)
    actions = await engine.backtest_indicators(
        tickers, args.start_date, args.end_date, args.indicators
    )

    # Save results
    engine.save_to_csv(actions, args.output)

    # Print summary
    print("\n🎯 OPTIMIZED PROFITABLE STRATEGY BACKTEST COMPLETED!")
    print(f"Total actions generated: {len(actions)}")
    print("🎯 Goal: High quality profitable trades (fewer but better)")

    if actions:
        # Count by indicator
        indicator_counts = {}
        ticker_counts = {}
        entry_actions = 0
        exit_actions = 0
        total_pnl = 0
        
        for action in actions:
            indicator_counts[action.indicator] = indicator_counts.get(action.indicator, 0) + 1
            ticker_counts[action.ticker] = ticker_counts.get(action.ticker, 0) + 1
            if action.signal_type == "entry":
                entry_actions += 1
            else:
                exit_actions += 1
                total_pnl += action.pnl

        print("\n📊 Optimized Actions by indicator:")
        for indicator, count in sorted(indicator_counts.items()):
            print(f"  {indicator}: {count}")

        print("\n📈 Optimized Actions by ticker:")
        for ticker, count in sorted(ticker_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
            print(f"  {ticker}: {count}")
            
        print(f"\n📊 Trade Summary:")
        print(f"  Entry actions: {entry_actions}")
        print(f"  Exit actions: {exit_actions}")
        print(f"  Complete trades: {exit_actions}")
        print(f"  Total P&L: ${total_pnl:.2f}")
        
        if exit_actions > 0:
            avg_pnl = total_pnl / exit_actions
            win_rate = sum(1 for a in actions if a.signal_type == "exit" and a.pnl > 0) / exit_actions
            print(f"  Average P&L per trade: ${avg_pnl:.2f}")
            print(f"  Win rate: {win_rate:.1%}")
        
        # Price range
        prices = [a.price for a in actions]
        if prices:
            print(f"  Price range: ${min(prices):.2f} - ${max(prices):.2f}")
            
        # Date range
        timestamps = [a.timestamp for a in actions]
        if timestamps:
            print(f"  Date range: {min(timestamps)[:10]} to {max(timestamps)[:10]}")
    else:
        print("\n⚠️  No optimized actions generated - try different parameters")


if __name__ == "__main__":
    asyncio.run(main())
