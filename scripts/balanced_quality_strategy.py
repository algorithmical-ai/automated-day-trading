#!/usr/bin/env python3
"""
Balanced Quality Profitable Trading Strategy with Complete Trade Pairs

This script implements a balanced quality-focused profitable trading strategy that:
...
4. Balances quality with signal generation
"""
...
class BalancedQualityStrategy:
    """Balanced quality profitable trading strategy"""
    ...
    def simulate_balanced_quality_trades(
        self,
        ticker: str,
        bars: List[Dict[str, Any]],
    ) -> List[TradeAction]:
        """Simulate balanced quality trades with complete entry/exit pairs"""
        actions = []

        if len(bars) < 50:
            return actions

        print(f"Simulating Balanced Quality for {ticker} with {len(bars)} bars")
        ...
        for i in range(30, len(bars)):
            ...
            entry_signal = self._check_balanced_entry(
            ...
            for trade_id, trade_info in active_trades.items():
                exit_signal = self._check_balanced_exit(
    ...
    def _check_balanced_entry(
        ...
        """Check for balanced entry conditions"""
        
        # Max 3 quality trades per day
        ...
        if abs(momentum_score) < 0.3:  # 0.3% minimum momentum
            return None

        # Volume confirmation
        volume_analysis = self._analyze_balanced_volume(volumes, i)
        ...
        if momentum_score > 0:  # Long entry
            if (tech_analysis["rsi"] >= 35.0 and tech_analysis["rsi"] <= 75.0 and  # Wider RSI range
                tech_analysis["trend_strength"] >= 0.3 and  # Moderate uptrend
                tech_analysis["volatility"] >= 0.005 and  # Minimum volatility
                tech_analysis["price_position"] >= 0.4):  # Mid-to-upper range position
                
                # Additional balanced filters
                if self._balanced_entry_filters(prices, volumes, i, is_long=True):
                    return {
                        "action": "buy_to_open",
                        "reason": f"BALANCED Quality long: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x",
            ...
            if (tech_analysis["rsi"] >= 55.0 and tech_analysis["rsi"] <= 90.0 and  # Wider overbought range
                tech_analysis["trend_strength"] <= -0.3 and  # Moderate downtrend
                tech_analysis["volatility"] >= 0.005 and  # Minimum volatility
                tech_analysis["price_position"] <= 0.6):  # Mid-to-lower range position
                
                # Additional balanced filters
                if self._balanced_entry_filters(prices, volumes, i, is_long=False):
                    return {
                        "action": "sell_to_open",
                        "reason": f"BALANCED Quality short: score={momentum_score:.2f}, rsi={tech_analysis['rsi']:.1f}, trend={tech_analysis['trend_strength']:.2f}, vol={volume_analysis['volume_ratio']:.1f}x",
                        "confidence": min(abs(momentum_score) / 1.5, 1.0),
                        "technical_indicators": {
                            "momentum_score": momentum_score,
                            "rsi": tech_analysis["rsi"],
    ...
    def _check_balanced_exit(
        ...
        """Check for balanced exit conditions"""
        ...
        # Time-based exit (3-10 minutes for balanced trades)
        time_held = i - entry_time
        if time_held >= 10:  # Max 10 minutes
        ...
        # Profit target (2% profit for balanced trades)
        if pnl_pct >= 2.0:
        ...
        # Stop loss (1% loss for balanced trades)
        if pnl_pct <= -1.0:
    ...
    def _calculate_balanced_momentum(
        ...
        # Volume-weighted momentum (balanced)
    ...
    def _analyze_balanced_volume(self, volumes: List[float], i: int) -> Dict[str, float]:
    ...
    def _balanced_entry_filters(self, prices: List[float], volumes: List[float], i: int, is_long: bool) -> bool:
        """Balanced entry filters"""
        if i < 10:
            return False
        ...
        if volume_consistency < 0.2:  # Some volume consistency required
            return False
        ...
        if is_long:
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            return momentum_2 > 0.1 and momentum_5 > 0.15  # Moderate positive momentum
        else:
            momentum_2 = (prices[i] - prices[i-2]) / prices[i-2] * 100 if i >= 2 else 0
            momentum_5 = (prices[i] - prices[i-5]) / prices[i-5] * 100 if i >= 5 else 0
            return momentum_2 < -0.1 and momentum_5 < -0.15  # Moderate negative momentum
    ...
    async def backtest_indicators(
        ...
        """Run balanced quality backtest"""
        print(f"Starting BALANCED QUALITY backtest from {start_date} to {end_date}")
        print(f"Tickers: {', '.join(tickers)}")
        print(f"Indicators: {', '.join(indicators)}")
        print("🎯 FOCUS: Balanced quality profitable trades")
        ...
        for ticker in tickers:
            ...
            if "Momentum" in indicators:
                momentum_actions = self.simulate_balanced_quality_trades(ticker, bars)
                print(f"  Balanced Quality: {len(momentum_actions)} actions")
    ...
    def save_to_csv(self, actions: List[TradeAction], filename: str):
        ...
        print(f"Saved {len(actions)} BALANCED QUALITY actions to {filename}")


async def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Balanced quality profitable strategy backtest")
    ...
    parser.add_argument(
        "--output",
        default="balanced_quality_profitable_trading_2021_2026.csv",
    ...
    if not args.tickers:
        engine = BalancedQualityStrategy(api_key, secret_key)
    ...
    # Print summary
    print("\n🎯 BALANCED QUALITY PROFITABLE STRATEGY BACKTEST COMPLETED!")
    print(f"Total actions generated: {len(actions)}")
    print("🎯 Goal: Balanced quality profitable trades")
    ...
    if actions:
        ...
        print("\n⚠️  No balanced quality actions generated - try different parameters")
