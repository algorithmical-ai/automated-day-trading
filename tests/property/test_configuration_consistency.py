...
def test_property_77_stop_loss_atr_multiplier_consistency(atr: float, price: float):
    ...
    # The expected stop loss should use exactly 2.0x ATR
    assert ATR_STOP_LOSS_MULTIPLIER == 2.0, (
        f"Stop loss ATR multiplier should be 2.0, got {ATR_STOP_LOSS_MULTIPLIER}"
    )
    
    # Verify the calculation is consistent
    calculated_stop_loss = atr_percent * 2.0
...
def test_property_78_trailing_stop_atr_multiplier_consistency(
...
def test_all_indicators_import_from_trading_config():
    ...
    assert ATR_STOP_LOSS_MULTIPLIER == 2.0
    assert ATR_TRAILING_STOP_MULTIPLIER == 1.5
    assert PENNY_STOCK_STOP_LOSS_MIN == -8.0
    assert PENNY_STOCK_STOP_LOSS_MAX == -4.0
