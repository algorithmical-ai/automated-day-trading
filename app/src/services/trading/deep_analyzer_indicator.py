    ...
    async def entry_service(cls):
        ...
        while cls.running:
            try:
                ...
                ticker_candidates = []  # List of (ticker, entry_score, action, signal_data, reason)
                market_data_dict = {}
                ...
                long_signal_lookup = {
                    t: (signal, reason)
                    for t, _, _, signal, reason in long_candidates
                }
                short_signal_lookup = {
                    t: (signal, reason)
                    for t, _, _, signal, reason in short_candidates
                }
                ...
                for rank, (ticker, entry_score, _) in enumerate(top_long, start=1):
                    ...
                    ranked_reason = f"{reason} (ranked #{rank} long, entry_score: {entry_score:.2f})"
                    is_golden = signal_data.get("is_golden", False) if signal_data else False
                    portfolio_allocation = (
                        signal_data.get("portfolio_allocation", None)
                        if signal_data
                        else None
                    )
                ...
                for rank, (ticker, entry_score, _) in enumerate(top_short, start=1):
                    ...
                    ranked_reason = f"{reason} (ranked #{rank} short, entry_score: {entry_score:.2f})"
                    is_golden = signal_data.get("is_golden", False) if signal_data else False
                    portfolio_allocation = (
                        signal_data.get("portfolio_allocation", None)
                        if signal_data
            ...
            try:
                ...
                for trade in active_trades:
                    ...
                    if should_exit:
                        ...
                        if exit_price <= 0:
                            logger.warning(
                                f"Failed to get valid exit price for {ticker}"
                            )
                            continue
