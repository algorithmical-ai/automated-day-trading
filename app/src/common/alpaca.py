...
class AlpacaClient:
    ...
    async def cleanup_session(cls):
        """Clean up the shared session. Call this when shutting down the application."""
        async with cls._session_lock:
            if cls._session and not cls._session.closed:
                await cls._session.close()
                cls._session = None
                logger.debug("Closed shared Alpaca API session")

    @classmethod
    async def _get_session(cls) -> aiohttp.ClientSession:
        """
        Get or create a shared aiohttp session for connection pooling.
        This prevents connection reset issues with concurrent requests.
        """
        async with cls._session_lock:
            if cls._session is None or cls._session.closed:
                # Create optimized connector for Alpaca API
                connector = aiohttp.TCPConnector(
                    limit=8,  # Allow concurrent requests from entry+exit cycles
                    limit_per_host=5,  # Allow multiple concurrent requests to Alpaca
                ...
                logger.debug("Created new shared Alpaca API session")

            return cls._session

    @classmethod
    async def _is_clock_cache_valid(cls) -> bool:
        """
    ...
    async def clock(cls) -> Dict[str, Any]:
        ...
        async with cls._clock_cache_lock:
            ...
            if await cls._is_clock_cache_valid():
                cache_age = (
                    ...
                    async with session.get(url, headers=headers) as response:
                        ...
                        if response.status == 429:
                            if attempt < max_retries - 1:
                                ...
                                raise ValueError(
                                    f"Failed to fetch market clock after {max_retries} retries: rate limited"
                                )
                        ...
                        raise ValueError(
                            f"Failed to fetch market clock: {response.status}"
                        )
                except aiohttp.ClientError as e:
                    if attempt < max_retries - 1:
                        logger.warning(
                            f"Network error fetching market clock. Retrying in {retry_delay}s... (attempt {attempt + 1}/{max_retries})"
                        )
                        await asyncio.sleep(retry_delay)
                        continue
                    else:
                        logger.error(
                            f"Error fetching market clock after {max_retries} attempts: {str(e)}",
                            exc_info=True,
                        )
                        raise ValueError(
                            f"Error fetching market clock: {str(e)}"
                        ) from e
    ...
    async def is_shortable(cls, ticker: str) -> bool:
        ...
        if not cls.API_KEY_ID or not cls.API_SECRET_KEY:
            logger.warning(
                "Alpaca API credentials not configured, assuming ticker is not shortable"
            )
            return False
            ...
            try:
                ...
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        logger.debug(
                            f"Ticker {ticker} not found in Alpaca assets, assuming not shortable"
                        )
                        return False
                    else:
                        error_text = await response.text()
